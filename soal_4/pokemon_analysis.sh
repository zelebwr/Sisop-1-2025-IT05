...
FIND_VALUE=""
FIND_COLUMN1=1  # First search column
FIND_COLUMN2=1  # Second search column (default: same as first)
USE_SECOND_COLUMN=0  # Flag to check if the second column is used
			...
			if [[ -z "$2" || "$2" =~ ^- ]]; then
				...
				while [ $# -gt 0 ]; do
					case "$1" in
						...
						-c|--column)
							...
							if [[ -z "$2" || "$2" =~ ^- ]]; then 
								...
								if [[ "$2" =~ ^[0-9]+,[0-9]+$ ]]; then
									...
									echo "Try: -c 2,3 to refer to search based on column 2 and 3"
									echo "Try using -h or --help for help."
									exit 1;
								fi
							fi
						...
						-o|--output)
							if [[ -z "$2" || "$2" =~ ^- ]]; then 
								echo "Error: Output type must be specified after -o."
								echo "Try: -o row"
								echo "Try: -o focused"
								exit 1
							else 
								if [[ "$2" == "r" || "$2" == "row" || "$2" == "f" || "$2" == "focused" ]]; then
									...
									echo "Error: Invalid output type '$2'."
									echo "Try: -o row"
									echo "Try: -o focused"
									exit 1
								fi
